...
本周任务是在第06周的容器化项目基础上，集成 Nacos 实现服务的自动注册与发现，替代硬编码的服务地址。

## 核心任务

1.  **使用 Docker 启动 Nacos 服务器 (单机模式)**
2.  **为 User Service, Catalog Service, 和 Enrollment Service 添加 Nacos Discovery 依赖**
3.  **配置服务自动注册到 Nacos** (应用名、服务端口、命名空间等)
4.  **修改 Enrollment 服务**，使用 `DiscoveryClient` + `RestTemplate` 通过服务名调用 User Service 和 Catalog Service，不再使用硬编码地址
5.  **配置健康检查接口**，确保 Nacos 能够准确监控服务状态
6.  **在 Nacos 控制台验证服务注册成功**，查看实例列表和健康状态
7.  **启动多个服务实例**，验证负载均衡是否生效
8.  **提交后执行 git tag v1.1.0**
...
*   实现优雅上下线：服务关闭前先从 Nacos 注销，等待正在处理的请求完成后再关闭。
...
*   **可能原因**: Nacos 服务器未启动或网络不通；配置文件中的 `server-addr` 地址错误；命名空间 ID 不存在。
*   **解决方法**:
...
*   **可能原因**: 服务名配置错误（大小写敏感）；目标服务未启动或未注册成功；网络配置问题。
*   **解决方法**:
...
*   **可能原因**: 健康检查接口返回非 200 状态码；应用未完全启动；Actuator 端点未暴露。
*   **解决方法**:
