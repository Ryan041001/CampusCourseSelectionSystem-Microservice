...
必须完成 **hw04b**: 具备课程、学生、选课的完整功能，并已实现数据库持久化。
...
1. **catalog-service (课程目录服务): 8081**
   - Course (课程管理)

2. **enrollment-service (选课服务): 8082**
   - Student (学生管理)
   - Enrollment (选课管理, 调用 catalog-service 验证课程)

### 1.1 创建课程目录服务 (catalog-service) (25分)
...
### 1.2 创建选课服务 (enrollment-service) (25分)
...
**服务间通信实现 (`EnrollmentService.java`):**
...
**配置文件 (`enrollment-service/application.yml`):**
...
## 第二部分: Docker 容器化部署 (30分)

### 2.1 编写 Dockerfile (10分)
...
### 2.2 编写 docker-compose.yml (20分)

创建完整的 `docker-compose.yml` 文件，包含所有服务。

**必须包含的服务:**

- `catalog-db` - MySQL数据库 (课程目录服务)
- `enrollment-db` - MySQL 数据库 (选课服务)
- `catalog-service` - 课程目录微服务
- `enrollment-service` - 选课微服务
...
services:
  ...
  catalog-service:
    ...
    environment:
      ...
      DB_URL: jdbc:mysql://catalog-db:3306/catalog_db?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true
  ...
  enrollment-service:
    ...
    environment:
      SPRING_PROFILES_ACTIVE: prod
      DB_URL: jdbc:mysql://enrollment-db:3306/enrollment_db?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true
...
- **项目简介**: 名称、版本、基于哪个版本拆分、微服务架构说明
- **架构图**: 描述服务、数据库及 HTTP 调用关系
- **技术栈**: Spring Boot 3.5.7, Java 25, MySQL 8.4, Docker, RestTemplate
- **环境要求**: JDK 25+, Maven 3.8+, Docker 20.10+, Docker Compose 2.0+
- **构建和运行步骤**
- **API 文档** (列出所有接口)
- **测试说明**
- **遇到的问题和解决方案**
...
│   ├── src/main/java/com/zjgsu/<你的姓名缩写>/catalog/
│   │   ├── model/
│   │   ├── repository/
│   │   ├── service/
│   │   ├── controller/
...
└── enrollment-service/     # 选课服务
    ├── src/main/java/com/zjgsu/<你的姓名缩写>/enrollment/
    │   ├── model/
    │   ├── repository/
    │   ├── service/
    │   ├── controller/
...
```bash
# 创建版本标签
git tag -a v1.2.0-m -m "Release version 1.2.0 Microservices architecture"
git push origin v1.2.0-m
```
...
- **版本标签**: `v1.0.0` (注: 文档中此处写v1.0.0，但上方Git命令是v1.2.0-m，请以实际要求为准，建议保持一致)
- **分支**: `main`
- **运行命令**:
...
- **catalog-service (25分)**: 代码完整性(10)、端口/DB配置(5)、API正确性(10)
- **enrollment-service (25分)**: 代码完整性(10)、服务间HTTP调用(10)、业务逻辑正确性(5)
- **服务间通信 (10分)**: RestTemplate配置(3)、课程验证逻辑(4)、异常处理(3)

### Docker 部署 (30分)

- **Dockerfile (10分)**: catalog(5), enrollment(5)
- **docker-compose.yml (20分)**: 数据库配置(8)、服务配置(6)、网络/卷(3)、依赖关系(3)

### 测试和文档 (10分)

- **功能测试 (5分)**: 脚本/步骤(3)、覆盖度(2)
- **README (5分)**: 完整性(3)、架构清晰(2)

### 加分项 (最多+15分)

- 健康检查配置 (+3分)
- 使用多阶段构建 Dockerfile (+3分)
- 实现课程容量自动更新 (+3分)
- 添加日志记录 (+2分)
- 性能优化 (+2分)
- Swagger/OpenAPI 文档 (+2分)
...
**Q2: 服务间调用失败怎么办?**
**A: 检查清单:**
- 确认 `catalog-service` 已启动: `docker logs catalog-service`
- 检查网络连接: `docker network inspect course-cloud_course-network`
- 测试 `catalog-service` 接口: `curl http://localhost:8081/api/courses`
- 检查环境变量: `docker exec enrollment-service env | grep CATALOG`

**Q3: 数据库初始化失败?**
**A: 解决方案:**
- 检查数据库容器状态: `docker-compose ps`
- 查看数据库日志: `docker-compose logs catalog-db`
- 手动连接数据库测试
- 重建数据卷: `docker-compose down -v` 然后 `docker-compose up -d`

**Q4: 如何调试服务间调用?**
**A:** 添加详细日志 (使用 `@Slf4j`):
记录开始选课参数、调用URL、服务响应内容以及异常信息。
...
- **先本地测试**: 使用 H2 内存数据库快速验证逻辑，本地启动两个服务进行测试，确认服务间调用正常后再容器化。
- **逐步容器化**: 先容器化 `catalog-service`，验证成功后再容器化 `enrollment-service`，最后测试服务间通信。
- **版本管理**: 每完成一个重要步骤就提交代码。
- **配置管理**: 使用 Spring Profile 区分开发和 Docker 环境。

### 思考题

- 如果 `catalog-service` 宕机，`enrollment-service` 会如何? 如何提高可用性?
- 如何实现课程人数的实时更新?
- 如果需要添加教师服务 (`teacher-service`)，应该如何拆分?
- 如何避免选课时的并发问题 (超选)?